...
**15 浜のマスクの 3 ゲート**（赤=岸線からの距離／緑=湖面からの高さ／青=傾き）。
...
**15 も同じ理由で足した（2026-09-04）。** 「水際の帯の幅を λ6m で振れ」という指示が 3 ラウンド効かなかったのは、
**幅を決めていたのが距離ゲートではなく高さゲートだった**から。距離ゲートを 100m にしても帯の幅は 96→102px しか
動かなかった。**「どのゲートが効いているか」を先に画で見る。**
...
## 担当の作業のしかた（毎回同じことを言わずに済むよう、ここに置く）

**この機は 1 台を全担当で分け合っている。** 撮影 1 枚に 30〜40 秒かかることがある。

- **1 コマンドで撮るのは 1 枚まで。`--all`（16 定点）は使わない。** 10 分を超えると見張りに「止まった」と判定される。
  **16 定点の通しは統合担当がやる。**各担当は自分の合否条件に必要な 1〜3 枚だけ撮る。
- 撮影が返ってこないときは待つ。**同じ撮影を重ねて投げない**（さらに混む）。
- **`git stash` は使わない。** リポジトリ共有なので、他担当の stash を誤って pop する事故が実際に起きた。控えはファイルに退避する。
- **`git add` は個別パスで。`git add -A` と `git add .` は作者が禁止している。**
- 作業前に `git merge feat/mathscape` を 1 回。他担当の変更が入っていると、測り直しが要ることがある。
- **区切りのいい所で、全部終わっていなくてもコミットして報告する。**途中まででも取り込める。

## 報告書の書き方（各担当がタスク終了時に返すもの）
1. ブランチ名とコミット
2. 変えたファイルと行
3. **犯人をどう特定したか**（`?dbg=` `?tdbg=` の切り分けの手順と、そのときの数字）
4. **直す前と後の実測値。** 合否条件の数字を必ず。**数字の無い報告は受け取らない**
5. 負荷: draw call と三角形数は決定的なので必ず。fps は混雑で当てにならないなら、構造（増えたタップ数など）で説明する
6. **直せなかったものと、その理由**（数字つきで。「やっていない」と正直に書く）
7. 他担当への申し送り／core への要望
...
- **`storm` の空の「大きな白い滲み」は空側（体積雲）。天気ではない。2026-09-04 天気担当より。**
  批評R7 の見立ては「雨のヴェールの塊（`weather/fog`）。λ を 1/3 に」だったが、切り分けると逆だった:
  `storm` の (0,30)-(1600,200) で「周囲（121px のならし）より 8bit で 12 以上明るく 2000px を超える塊」を数えると、
  **天気あり 3 個 / 6,689px → `weather.enabled = false` で 5 個 / 15,866px（増える）**。
  さらに `?dbg=nopost` の生の絵（露出前）にも同じ位置（x≈551 / 617 / 1250 / 1440）に明るい塊が出ている。
  つまり**雨のヴェールは滲みを半分以上隠している側**で、正体は雲。ポストの露出がそれを持ち上げている。
  天気側からは触っていない。空担当へ。
- **地形の `gAlb` は 1.0（物理値）。決着済み（2026-09-04）。** 一度 0.52 まで下げたが戻した。
  理由: 私（統合担当）が出した「近景 ÷ 空を 30〜50% に」という目標が誤りだった。
  日なたの草（約 6,400 cd/m²）は天頂の空（2,000〜4,000 cd/m²）より**明るい**のが物理で、
  写真で空が明るく見えるのは明度と色みの差である。空担当が 3 通り（間接光の内訳・立体角・π）で
  大気に係数の抜けが無いことを実測した。**アルベドで「比」を作らないこと。**
  経緯は `terrain/glsl.ts` の `gAlb` のコメントに全部書いてある。
...
- **遠景を直すために近景の値を動かしたら、必ず近景も撮る。** 距離帯が違えば求められるものも違う
  ...
  片方だけの数字で「直った」と報告しない。
- **自動露出の測光の重みは変えない（決着済み・2026-09-04）。** 「地面に偏っているのでは」と疑って実測した:
  下半分 : 上半分 = **1.69 : 1**（中央下寄りの評価測光として極端ではない）。
  一様（1.00 : 1）に差し替えて 16 定点を測ると、目的の `noon_side` は **−0.023 段しか動かず**、
  代わりに `cloudy_side` −0.36 / `forest` −0.27 / `dawn` −0.25 / `night` −0.22 / `golden` −0.18 段と暗い側が沈む。
  「アルベドを上げても近景の表示が動かず空だけ暗くなる」のは**測光の偏りではなく自動露出そのものの性質**で、
  重みを一様にしても地面が画面の過半を占める構図では同じことが起きる。
  **近景だけを下げたいときの道具は測光ではなく「近景の頭打ち（`uGroundCap` ＋やわらかい膝）」。**
