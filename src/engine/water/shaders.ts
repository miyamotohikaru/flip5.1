  ...
  for (int L = 0; L < 3; L++){
    ...
    for (int y = -1; y <= 1; y++){
      for (int x = -1; x <= 1; x++){
        ...
        float ampl = (1.0 - tt) * (1.0 - tt) * 0.0045;
        // 波面: sin(60 ring) の傾き
...
void main(){
  ...
  vec4 d0 = texture2D(tDeriv0, vWorld.xz / uTiles.x);
  vec4 d1 = texture2D(tDeriv1, vWorld.xz / uTiles.y);
  ...
  // 雨の波紋
  float rainNear = (1.0 - smoothstep(12.0, 60.0, dist)) * uRain;
  if (rainNear > 0.005) {
  ...
  if (!under) {
    ...
    vec3 Pb = vWorld + T * min(along, 60.0);
    vec4 cb = uViewProj * vec4(Pb, 1.0);
    vec2 ruv = cb.xy / max(cb.w, 1e-4) * 0.5 + 0.5;
    float zb = texture2D(tSceneDepth, ruv).r;
    if (zb < vViewZ + 0.05 || any(lessThan(ruv, vec2(0.0))) || any(greaterThan(ruv, vec2(1.0)))) { ruv = suv; zb = sceneZ; }
    ...
    // ---- 泡: 波頭（ヤコビアン）＋ 岸の寄せ波
    {
      ...
      float foamShore = shallow * (0.1 + 0.9 * wash) * smoothstep(0.0, 0.05, vDepthA) * (0.35 + 0.65 * smoothstep(1.0, 8.0, wind));
      ...
      float foam = smoothstep(0.55 - 0.5 * m, 0.75 - 0.3 * m, foamN * 0.8 + foamN2 * 0.2 + m * 0.35) * m;
      ...
      float edge = smoothstep(0.0, ew, vertEdge) * smoothstep(0.0, 0.15, along);
  ...
  if (fm > 0.0) {
    ...
    for (int i = 0; i < 5; i++){
      ...
      float vis = (1.0 - smoothstep(0.12, 0.35, dens)) * (i == 2 ? 0.6 : 0.32);
      lines += flip_line(ph, 0.03) * vis;
    }
    ...
    // 実際の変位の等高線（波の高さ。間隔は有義波高の 1/3）
    {
      ...
      fc += FLIP_LINE * 0.7 * flip_line(hv + 0.5, 0.06) * vis;
    }
    // 風向の矢印の格子
    ...
    if (uWaterA.x > 0.5 && !under) {
      ...
      fc += texture2DLodEXT(tReflection, uv0, 0.0).rgb * 0.45;
    }
    fc += FLIP_ACCENT * flip_edgeGlow(vWorld) * 1.5;
    fc = flip_applyAerial(fc, vWorld) * 0.6 + fc * 0.4;
    col = mix(col, fc, fm);
  }
