...
// エアロゾルが多い（ω0≈0.9）大気では過大になる。だから 1.0 未満にしている。
//
// 2026-09-04 に 0.5 / 0.75 / 1.0 を実測して 0.75 にした（?dbg=skyms: で再現できる）:
//
//   MS    noon_side 近景   空      近景÷空   dawn の地平の帯 R−G   夕景 R−G   夕景の彩度
//   0.5   0.2045          0.2831   72.2%     32                    36         0.577
//   0.75  0.2022          0.3146   64.3%     30                    33         0.519
//   1.0   0.1990          0.3331   59.7%     29 ← 合否(≥30)を割る   31         0.472
//
// 0.5 では空が 0.2831 で、実写の晴天（表示線形 0.35〜0.60）に対して暗すぎた。
// 1.0 は dawn の朝焼けを割り、夕景の彩度も 18% 落とす。0.75 が両立する。
#ifndef FLIP_MS
#define FLIP_MS 0.75
#endif
