...
void main(){
  ...
  vec3 sunLight = uSunE * Tv * (disc * limb * 260.0 + 0.010 * exp(-sunAng / 0.022));
  ...
  if (uStarVeil > 0.001 && d.y > -0.05){
    ...
    starLight += vec3(0.95, 0.9, 0.85) * mw * 0.0045;
  ...
  if (fm > 0.0){
    ...
    fc += FLIP_LINE * 0.5 * flip_line(lv, 0.04);
    // 太陽からの角度 10° 刻み（薄く）
    fc += FLIP_LINE * 0.14 * flip_line(degrees(sunAng) / 10.0, 0.025);
    // 天球の格子（赤緯・赤経 15°）
    vec3 e = uStarFrame * d;
    float dec = degrees(asin(clamp(e.z, -1.0, 1.0)));
    float ra = degrees(atan(e.y, e.x));
    float grid = max(flip_line(dec / 15.0, 0.02), flip_line(ra / 15.0, 0.02) * smoothstep(0.0, 0.2, 1.0 - abs(e.z)));
    ...
    if (d.y > 0.01){
      ...
      for (int k = 0; k < 3; k++){
        ...
        float iso = flip_line(dens * 5.0, 0.06) * step(0.02, dens);
