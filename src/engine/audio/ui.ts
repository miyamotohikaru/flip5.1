...
export function playEnter(ctx: Ctx, dest: AudioNode, res: Resources, t: number) {
  ...
  freqs.forEach((f, i) => {
    ...
    og.gain.linearRampToValueAtTime(i === 3 ? 0.025 : 0.06, t + 0.5);
...
export function playShutter(ctx: Ctx, dest: AudioNode, res: Resources, t: number) {
  ...
  // ミラーアップ
  click(t, 0.32, 2500);
  ring(t, 0.14, 2300, 18, 0.04);
  thump(t + 0.002, 0.2, 500, 0.03);
  // 機構の「シュッ」
  ...
  bp.connect(g);
  g.connect(dest);
  n.stop(attackDecay(g.gain, t + 0.015, 0.05, 0.01, 0.05));
  // 幕が閉じる
  click(t + 0.05, 0.26, 1800);
  ring(t + 0.05, 0.12, 1800, 14, 0.05);
  thump(t + 0.052, 0.24, 300, 0.045);
  // ばねの余韻
  const o = ctx.createOscillator();
  o.type = "sine";
  ...
  const end = attackDecay(og.gain, t + 0.055, 0.02, 0.003, 0.12);
  o.start(t + 0.055);
  o.stop(end + 0.05);
}
...
export class FlipVoice {
  ...
  constructor(private ctx: Ctx, dest: AudioNode, res: Resources, t: number) {
    ...
    this.sweepBP = biquad(ctx, "bandpass", 140, 4);
    ...
    // 唸り（AM は掛け算: 0.7 ± 0.3 の段を挟んでから音量を掛ける）
    this.gDrone = gainNode(ctx, 0);
    const dlp = biquad(ctx, "lowpass", 220, 0.8);
    const dam = gainNode(ctx, 0.7);
    dlp.connect(dam);
    dam.connect(this.gDrone);
  ...
  update(p: number, dir: 1 | -1, flipView: number, t: number, complete: boolean) {
    ...
    // 全域が数式になったら、ゆっくり（1 秒ほどで）薄い持続へ落ち着く
    const tc = complete ? 0.9 : 0.08;
    setT(this.sweepBP.frequency, 140 * Math.pow(64, q), t, 0.06);
    const sweep = complete ? 0.06 * flipView : (dir > 0 ? 0.9 : 0.75) * Math.pow(mid, 0.6);
    setT(this.gSweep.gain, sweep, t, tc);
    const det = 1200 * q;
    for (const pt of this.partials) setT(pt.o.detune, det, t, 0.08);
    const shim = complete ? 0.08 * flipView : (dir > 0 ? 0.4 : 0.3) * Math.pow(q, 0.7);
    setT(this.gShim.gain, shim, t, complete ? tc : 0.1);
    const drone = complete ? 0.03 * flipView : 0.14 * Math.pow(mid, 0.7);
    setT(this.gDrone.gain, drone, t, complete ? tc : 0.15);
    setT(this.gTicks.gain, complete ? 0.03 * flipView : 0.22 * mid, t, tc);
  }

  stop(t: number) {
    if (this.stopped) return;
    this.stopped = true;
    for (const g of [this.gSweep, this.gShim, this.gDrone, this.gTicks]) setT(g.gain, 0, t, 0.12);
    const end = t + 0.6;
