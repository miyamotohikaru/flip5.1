...
function skyPanel(hit: Extract<ProbeHit, { kind: "sky" }>, compact: boolean): FormulaPanel {
  ...
  const lines = compact
    ? [
        ...
        `σᴹ(h) = 1.0e${MINUS}2·e^(${MINUS}h/2.5km) + 靄`,
        `pᴿ(θ) = 3/16π(1+cos²θ) = ${num(a.phaseR, 3)}`,
        `pᴹ(θ) = C–S(θ, g=0.86) = ${num(a.phaseM, 3)}`,
    ...
    : [
        ...
        `σᴹ(h) = 1.0e${MINUS}2 · e^(${MINUS}h/2.5 km) + 靄 ${num(a.haze, 3)}·e^(${MINUS}h/1 km)`,
        `pᴿ(θ) = 3/(16π)·(1 + cos²θ)                       = ${num(a.phaseR, 4)}`,
        `pᴹ(θ) = Cornette${MINUS}Shanks(θ, g = 0.86)               = ${num(a.phaseM, 4)}`,
