 ...
 *   ③ 樹冠の下（真上が林）… 距離に関係なく効く。地形の tCanopy を引き継ぐもので、
 *      これが無いと手前の林床が日なたと同じ明るさになる（②は近景では CSM に譲って弱めているため）。
 * camDist はカメラからの距離（m）。②（長い帯）は近景では CSM が本物の落ち影を出すので
 * uSunOccParams.zw で徐々に効かせる。③ は近景でこそ要るので距離で切らない。
 */
