...
const sky: Formula[] = [
  {
    ...
    steps: [
      ...
      {
        note: "直し1: ミーと靄を上げた → それでも白いまま（散乱を増やすと分母も増えて相殺する）",
        failed: true,
        body: E(`σ^{M}(h) = 1.0·10^{-2}·e^{-h/2.5km} + haze(h)`),
      },
      {
        note: "直し2: 青を「吸収」する成分を分けたら橙になった（散乱ではなく吸収の波長依存）",
        body: E(`σ^{M}_{a}(h) = 4.0·10^{-3}·(0.43, 1.0, 2.44)·e^{-h/2.5km}`),
      },
      { note: "位相関数: レイリーは前後対称", body: E(`p^{R}(θ) = \\f{3}{16π}(1 + \\F{cos}{θ}^{2})`) },
      {
        note: "ミーは前方に強い（g = 0.86）",
